// -----------------------------------------------------------------------
// <copyright file="TimerNodeYamlConverter.cs" company="FuzzyPotato">
...
namespace FuzzyPotato.Core.Tests.Examples.Nodes.Converters
{
    ...
    /// <summary>
    /// YAML converter for TimerNode.
    /// </summary>
    public class TimerNodeYamlConverter : IYamlTypeConverter
    {
        /// <inheritdoc/>
        public bool Accepts(Type type)
        {
            return type == typeof(TimerNode);
        }
                ...
                if (parser.Current is Scalar scalar)
                {
                    ...
                    switch (propertyName)
                    {
                        ...
                        case "typeName":
                            ...
                            break;
                        case "schedule":
                            node.Schedule = parser.Consume<Scalar>().Value;
                            break;
                        case "triggerOnStart":
                            node.TriggerOnStart = bool.Parse(parser.Consume<Scalar>().Value);
                            break;
                        case "timeZone":
                            node.TimeZone = parser.Consume<Scalar>().Value;
                            break;
                        case "isEnabled":
                            node.IsEnabled = bool.Parse(parser.Consume<Scalar>().Value);
                            break;
                        case "nodeId":
        ...
        {
            if (value is not TimerNode node)
            {
            ...
            emitter.Emit(new Scalar("schedule"));
            emitter.Emit(new Scalar(node.Schedule));
            emitter.Emit(new Scalar("triggerOnStart"));
            emitter.Emit(new Scalar(node.TriggerOnStart.ToString().ToLower()));
            emitter.Emit(new Scalar("timeZone"));
            emitter.Emit(new Scalar(node.TimeZone));
            emitter.Emit(new Scalar("isEnabled"));
            emitter.Emit(new Scalar(node.IsEnabled.ToString().ToLower()));
