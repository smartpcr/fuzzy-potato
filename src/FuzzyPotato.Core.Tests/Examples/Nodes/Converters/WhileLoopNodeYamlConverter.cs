// -----------------------------------------------------------------------
// <copyright file="WhileLoopNodeYamlConverter.cs" company="FuzzyPotato">
...
namespace FuzzyPotato.Core.Tests.Examples.Nodes.Converters
{
    ...
    /// <summary>
    /// YAML converter for WhileLoopNode.
    /// </summary>
    public class WhileLoopNodeYamlConverter : IYamlTypeConverter
    {
        /// <inheritdoc/>
        public bool Accepts(Type type)
        {
            return type == typeof(WhileLoopNode);
        }
                ...
                if (parser.Current is Scalar scalar)
                {
                    ...
                    switch (propertyName)
                    {
                        ...
                        case "loopBodyStartNodeId":
                            node.LoopBodyStartNodeId = parser.Consume<Scalar>().Value;
                            break;
                        case "condition":
                            node.Condition = parser.Consume<Scalar>().Value;
                            break;
                        case "maxIterations":
                            node.MaxIterations = int.Parse(parser.Consume<Scalar>().Value);
                            break;
                        case "breakOnError":
                            node.BreakOnError = bool.Parse(parser.Consume<Scalar>().Value);
                            break;
                        case "nodeId":
        ...
        {
            if (value is not WhileLoopNode node)
            {
            ...
            if (node.LoopBodyStartNodeId != null)
            {
                emitter.Emit(new Scalar("loopBodyStartNodeId"));
                emitter.Emit(new Scalar(node.LoopBodyStartNodeId));
            }

            emitter.Emit(new Scalar("condition"));
            emitter.Emit(new Scalar(node.Condition));
            emitter.Emit(new Scalar("maxIterations"));
            emitter.Emit(new Scalar(node.MaxIterations.ToString()));
            emitter.Emit(new Scalar("breakOnError"));
            emitter.Emit(new Scalar(node.BreakOnError.ToString().ToLower()));
