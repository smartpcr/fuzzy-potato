// -----------------------------------------------------------------------
// <copyright file="SwitchNodeJsonConverter.cs" company="FuzzyPotato">
...
namespace FuzzyPotato.Core.Tests.Examples.Nodes
{
    ...
    /// <summary>
    /// JSON converter for SwitchNode.
    /// </summary>
    public class SwitchNodeJsonConverter : JsonConverter<SwitchNode>
    {
        /// <inheritdoc/>
        public override SwitchNode? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            foreach (var property in root.EnumerateObject())
            {
                switch (property.Name)
                {
                    ...
                    case "typeName":
                        ...
                        break;
                    case "expression":
                        node.Expression = property.Value.GetString() ?? string.Empty;
                        break;
                    case "cases":
                        node.Cases = JsonSerializer.Deserialize<Dictionary<string, string>>(property.Value.GetRawText()) ?? new Dictionary<string, string>();
                        break;
                    case "defaultPort":
                        node.DefaultPort = property.Value.GetString() ?? string.Empty;
                        break;
                    case "caseSensitive":
                        node.CaseSensitive = property.Value.GetBoolean();
                        break;
                    case "nodeId":
        ...
        /// <inheritdoc/>
        public override void Write(Utf8JsonWriter writer, SwitchNode value, JsonSerializerOptions options)
        {
            ...
            writer.WriteString("defaultPort", value.DefaultPort);
